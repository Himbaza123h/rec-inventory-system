...
<div class="content-page">
    ...
    <div class="content">
                        ...
                        <div class="panel-heading">
                            <div class="row product-section" id="sunglasses-field" style="color: #ffffff;">
                                ...
                                <form method="POST" action="{{ route('admin.purchase.store') }}">
                                    ...
                                    @csrf
                                    <table id="datatable-buttons" class="table table-striped" style="color: #ffffff; margin-top:20px;">
                                        <thead>
                                            <tr>
                                                <th class="col-md-1">
                                                    N/O
                                        ...
                                        <tr>
                                            ...
                                            <td><input type="text" name="Qty_{{ $item->id }}" class="form-control qty-input" placeholder="Enter Qty" data-item-id="{{ $item->id }}"></td>
                                            <td><input type="text" name="price_{{ $item->id }}" class="form-control" placeholder="Enter purchase price"></td>
                                            <td><input type="hidden" name="selected[]" value="{{ $item->id }}" class="checkbox"></td>
                                        </tr>
                                        @endforeach
                                        <!-- </tbody> -->
                                    </table>
                                    <center>
                                        <br><button type="submit" class="btn btn-primary waves-effect waves-light">ADD
                            ...
                            <div class="row product-section" id="lens-field" style="color: #ffffff;">
                                ...
                                <form method="POST" action="{{ route('admin.purchase.lens.store') }}">
                                    <input type="hidden" class="text-dark" name="purchase2Code" value="{{ $randomCode1 }}" readonly><br>
                                    @csrf
                                    <table id="datatable-buttons" class="table table-striped" style="color: #ffffff;">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td><input type="text" name="Qty2_{{ $item->id }}" class="form-control qty-input" placeholder="Enter Qty" data-item-id="{{ $item->id }}"></td>
                                                <td><input type="text" name="price2_{{ $item->id }}" class="form-control" placeholder="Enter purchase price"></td>
                                                <td><input type="hidden" name="selected2[]" value="{{ $item->id }}" class="checkbox"></td>
                                            </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-success rounded p-2">
                                            <a href="{{ route('admin.purchase.edit', [$item->purchase_code]) }}" style="text-decoration: none" class="t-decoration-none text-white">{{ __('view') }}</a>
                                        </button>
                                        <button class="btn btn-danger waves-effect waves-light" id="removeItem"><a href="{{ route('admin.purchase.delete', ['id' => $item->purchase_code]) }}" onclick="event.preventDefault();
                                                                         if (confirm('Are you sure you want to remove this purchase?')) {
                                                                             document.getElementById('delete-form-{{ $item->purchase_code }}').submit();
                                                                         }" style="text-decoration: none" class="t-decoration-none text-white">
                                                Remove <i class="fa fa-minus"></i></a></button>
                                        <form id="delete-form-{{ $item->purchase_code }}" action="{{ route('admin.purchase.delete', ['id' => $item->purchase_code]) }}" method="POST" style="display: none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
        ...
        <div class="alert" id="message-show" style="margin-left: 20px; margin-right: 20px; margin-top: -65px;">
            <p>
            <h4 class="text-center" style="color: #000"><i class="fa fa-exclamation-triangle"></i> SELECT PRODUCT TO PURCHASE</h4>
            </p>
                        ...
                        <table id="datatable-buttons" class="table table-striped table-bordered">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-success rounded p-2">
                                            <a href="{{ route('admin.purchase.lens.edit', [$data->purchase_code]) }}" style="text-decoration: none" class="t-decoration-none text-white">{{ __('view') }}</a>
                                        </button>
                                        <button class="btn btn-danger waves-effect waves-light" id="removeItem"><a href="{{ route('admin.purchase.lens.delete', ['id' => $data->purchase_code]) }}" onclick="event.preventDefault();
                                                                         if (confirm('Are you sure you want to remove this purchase?')) {
                                                                             document.getElementById('delete-form-{{ $item->purchase_code }}').submit();
                                                                         }" style="text-decoration: none" class="t-decoration-none text-white">
                                                Remove <i class="fa fa-minus"></i></a></button>
                                        <form id="delete-form-{{ $data->purchase_code }}" action="{{ route('admin.purchase.lens.delete', ['id' => $data->purchase_code]) }}" method="POST" style="display: none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
